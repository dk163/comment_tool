...
public class Controller {
	...
	private void createModify(int category, HeaderFileModel headerFileModel, String begin, String end, String sig) {
		center("Modifications on Features list / Changes Request / Problems Report", begin, end);

		cLine(sig, begin, end);

		createTable(headerFileModel.getMModifDate(), headerFileModel.getMAuthor(), headerFileModel.getMKey(),
				headerFileModel.getMComment(), begin, end, sig);
	}
	...
	private void createFileHeader(int category, HeaderFileModel headerFileModel, String begin, String end, String sig) {
		...
		for (int i = 0; i < 80 - begin.length() - end.length() - "Date:".length() - headerFileModel.getMDate().length()
				- 1; i++) {
			this.mResult.append(" ");
		}
		...
		center("Copyright " + sdf.format(new Date()) + " TCL Communication Technology Holdings Limited.", begin, end);
		cLine(" ", begin, end);
		cStrLine("This material is company confidential, cannot be reproduced in any form", begin, end);
		cStrLine("without the written permission of TCL Communication Technology Holdings", begin, end);
		...
		appendUnknowLength(" Reference documents : ", headerFileModel.getMReferenceDoc(), begin, end);
		cLine(sig, begin, end);

		appendUnknowLength(" Comments : ", headerFileModel.getMComments(), begin, end);

		appendUnknowLength(" File     : ", headerFileModel.getMFile(), begin, end);

		appendUnknowLength(" Labels   : ", headerFileModel.getMLabels(), begin, end);
	...
	private void createHeader(int category, HeaderFileModel headerFileModel, String begin, String end, String sig) {
		...
		case 2:
			if (!begin.equals("|")) {
				cLineNot(begin.substring(begin.length() - 1, begin.length()), begin, end);
			}
			createFileHeader(category, headerFileModel, begin, end, sig);
			if (category != 2) {
				break;
	...
	private void cStrLine(String content, String begin, String end) {
		...
		for (int i = 0; i < hight; i++) {
			...
			if (i != hight - 1) {
				this.mResult.append(content.substring(i * shouldLength, shouldLength + i * shouldLength));
			} else {
				String last = content.substring(i * shouldLength, content.length());
	...
	private void createTable(String modifDate, String author, String key, String comment, String begin, String end,
			String sig) {
		begin = begin + " ";
		end = " " + end;
		int dateLengh = 11;
		int partLen = 23;
		int part4 = 80 - partLen * 2 - 11 - end.length() - begin.length();
		...
		cLine(sig, partLen - 1);
		this.mResult.append("|");
		cLine(sig, 80 - partLen * 2 - 11 - end.length() - begin.length());
		this.mResult.append(end + "\n");
	}
	...
	private void center(String string, String begin, String end) {
		this.mResult.append(begin);
		int blank = 80 - begin.length() - end.length() - string.length();
		for (int i = 0; i < blank / 2; i++) {
			this.mResult.append(" ");
		}
		this.mResult.append(string);
	...
	private void appendUnknowLength(String title, String content, String begin, String end) {
		...
		int shouldLength = 80 - begin.length() - end.length() - title.length();
	...
	public String getNote(String cType, String operate, int lineNum, String type, NoteModel noteModel) {
		...
		this.mResult.append(begin);
		if (lineNum == 1) {
			this.mResult.append("[" + cType + "]-" + operate + " by " /* + "by SCDTABLET." */ + noteModel.getMAuthor()
					+ "," + noteModel.getMDate());
			...
			this.mResult.append("[" + cType + "]-" + operate + "-BEGIN by " /* + "by SCDTABLET." */
					+ noteModel.getMAuthor() + "," + noteModel.getMDate());
			if (!noteModel.getMBugNum().trim().equals("")) {
				this.mResult.append("," + noteModel.getMBugNum());
			}
			this.mResult.append(",");
			...
			this.mResult.append(begin + "[" + cType + "]-" + operate + "-END by " /* + "by SCDTABLET." */
	...
	public void createRecord() {
		...
		for (int i = 0; i < j; i++) {
			String line = arrayOfString[i];
			if ((line.contains("date")) && (line.contains("author")) && (line.contains("Key"))
